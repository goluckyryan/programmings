...
G4VPhysicalVolume* DetectorConstruction::ConstructVolumes()
{
  ...
  G4Material* env_mat = new G4Material(name="Vaccum", density, ncomponents=3);
  env_mat->AddElement(e1Ar, fractionMass=1*perCent); 
  env_mat->AddElement(e1N, fractionMass=70*perCent); 
  env_mat->AddElement(e1O, fractionMass=29*perCent); 
  ...
  G4Box* targetShape = new G4Box("target", 0.5*20*mm, 0.5*20*mm,  1*mm);
  ...
  new G4PVPlacement(0, G4ThreeVector(0, 0, 0*mm), logicTarget, "target", fLBox, false, 0, checkOverlaps);                            
