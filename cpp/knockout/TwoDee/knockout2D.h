...
float* Knockout2D(float k, float theta_k, float theta_NN, float Sp){
    ...
    theta_k = atof(argv[3])/rad2deg;
    theta_NN = atof(argv[4])/rad2deg;
...
// ########################### Lab Frame Inversed Kinematics
    ...
    output[1] = Angle(P1L)/rad2deg; // theta_1
    output[2] = P2L[0] - mp; // T_2
    output[3] = Angle(P2L)/rad2deg; // theta_2
